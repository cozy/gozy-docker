...
sudo -b -u cozy sh -c '/usr/local/bin/cozy-stack config gen-keys /etc/cozy/credentials-key'
...
echo "Creating instance"
cozy-stack instances add --host 0.0.0.0 --apps drive,photos,collect,settings,onboarding --passphrase "$server_pass" "${instance_domain}"
